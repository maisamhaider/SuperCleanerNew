...
public class FragmentImages extends Fragment implements DuplicateListener {
    ...
    private void initUI() {
        ...
        getActivity().getWindow().setStatusBarColor(ContextCompat.getColor(getActivity(), R.color.color_yellow_sea_1));
        TextView_delete_click.setOnClickListener(v -> deleteDuplicate());

        new Handler(Looper.getMainLooper()).post(() -> getActivity().runOnUiThread(() -> {
    ...
    private void deleteDuplicate() {
        if (Constants.fileToBeDeleted.size() <= 0) {
            Functions.showToastMsg(getActivity(), getString(R.string.least_one_photo));
